...
def generate_svg_plot2(dictionary, min, max, liste_cord, width, choice):
	...
	if min >= 0 :
		...
		if choice == 'exponential-power':
			plt.plot(x, funcexpopower(x, a, b, c), '#26C4EC', label="Expo-Power Fitted Curve")
		...
		for func in dictionary.keys():
			if func == 'exp':
				...
				c = dictionary[func]['c']
		if choice == 'exponential':
			plt.plot(x, funcexp(xneg, a, b, c), '#401539', label="Exp Fitted Curve")
			...
			if func == 'quad':
				...
				c = dictionary[func]['c']
		if choice == 'quadratic':
			plt.plot(x, funcquad(xneg, a, b, c), '#458C8C', label="Quad Fitted Curve")
			...
			if func == 'pow':
				...
				c = dictionary[func]['c']
		if choice == 'power':
			plt.plot(x, funcpuis(xneg, a, b, c), '#6DA63C', label="Pow Fitted Curve")
			...
			if func == 'log':
				...
				d = dictionary[func]['d']
		if choice == 'logarithmic':
			plt.plot(x, funclog(xneg, a, b, c, d),'#D9585A', label="Log Fitted Curve")
		...
		if choice == 'linear':
			...
			if func =='expo-power':
				...
				c = dictionary[func]['c']
		if choice == 'exponential-power':
			plt.plot(x, funcexpopower(xneg, a, b, c),'#26C4EC', label="Expo-Power Fitted Curve")
